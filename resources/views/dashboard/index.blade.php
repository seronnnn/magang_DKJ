...
<div id="chart-modal-overlay"
  ...
  <div id="chart-modal-box"
    ...
    <div id="modal-loading" style="text-align:center;padding:40px;color:var(--muted);font-size:13px;display:none">
      <div style="font-size:24px;margin-bottom:8px">⏳</div>Loading data…
    </div>
    <div id="modal-table-wrap" style="border:1px solid var(--border);border-radius:10px;overflow:hidden;display:none">
...
<div class="grid-kpi-5" style="margin-bottom:20px">
  ...
  <div class="kpi-card card-accent-yellow">
    ...
    <div class="kpi-value" style="color:{{ $collectionRate === null ? '#94a3b8' : ($collectionRate >= 100 ? '#16a34a' : ($collectionRate >= 70 ? '#d97706' : '#dc2626')) }}">
...
<div class="grid-charts-3" style="margin-bottom:20px">
  <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:18px;box-shadow:var(--shadow)">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
      ...
      <div style="font-size:10px;color:var(--muted)">👆 Click a slice</div>
    </div>
  ...
  <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:18px;box-shadow:var(--shadow)">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
      ...
      <div style="font-size:10px;color:var(--muted)">👆 Click a bar</div>
    </div>
      ...
      <table class="data-table">
        ...
        <tr>
          <td style="max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="{{ $r->customer_name }}">
            <a href="{{ route('dashboard.customers') }}?search={{ urlencode($r->customer_id) }}&period_id={{ $period?->id }}"
               style="color:var(--navy);text-decoration:none;font-weight:600">{{ $r->customer_name }}</a>
          </td>
...
const allRows     = @json(collect($rows)->values());
...
new Chart(document.getElementById('agingChart'), {
  ...
  options: {
    ...
    plugins: {
      legend: { position:'bottom', labels:{ font:{size:10}, boxWidth:10, padding:10, color:'#4a6080' },
                onClick: (e, li) => showAgingBucketModal(li.index) },
      tooltip: { callbacks:{ label: ctx=>' '+fmtIDR(ctx.raw) } }
    },
    onClick: (evt, els) => { if(els.length) showAgingBucketModal(els[0].index); }
  }
});
...
function showAgingBucketModal(idx){
  ...
  openModal(); showModalLoading();

  const params = new URLSearchParams(window.location.search);
  params.set('bucket', key);
  if (currentPeriodId) params.set('period_id', currentPeriodId);
  ...
  .then(r=>r.json())
  .then(data=>{
    const rows = data.rows, sum = data.bucket_sum;
    ...
    setModalHead(`<tr><th>#</th><th>Customer</th><th>Collector</th><th>Plant</th>
      <th class="num">${label}</th><th class="num">Total AR</th><th class="num">% of Bucket</th></tr>`);
    if(!rows.length){
  ...
  .catch(()=>{ setModalBody(`<tr><td colspan="7" style="text-align:center;padding:24px;color:#dc2626">⚠️ Failed to load.</td></tr>`); showModalTable(); });
}

/* ── 2. Collection by Collector ── */
...
new Chart(document.getElementById('collectorChart'), {
  ...
  options: {
    ...
    plugins:{ legend:{ labels:{ font:{size:10}, boxWidth:10 } }, tooltip:{ callbacks:{ label: ctx=>' '+fmtIDR(ctx.raw) } } },
    scales:{ x:{ ticks:{font:{size:10}} }, y:{ ticks:{ font:{size:9}, callback: v=>v>=1e9?'Rp '+(v/1e9).toFixed(1)+'B':'Rp '+(v/1e6).toFixed(0)+'M' } } },
    onClick: (evt, els) => { if(els.length) showCollectorModal(cNames[els[0].index]); }
  }
});
...
function showCollectorModal(name){
  ...
  setModalSummary(
    ...
    summaryCard('Collected', fmtIDR(cData.actual), cData.rate!==null?cData.rate+'% rate':null, '#16a34a') +
    summaryCard('Customers', rows.length, null, color)
  );
  setModalHead(`<tr><th>#</th><th>Customer</th><th>Plant</th>
    <th class="num">Total AR</th><th class="num">Target</th><th class="num">Actual</th><th class="num">Rate</th></tr>`);
  if(!rows.length){
...
new Chart(document.getElementById('plantChart'), {
  ...
  data: {
    ...
    datasets: [{ label:'AR Total', data: pNames.map(p=>byPlant[p].total), backgroundColor: pNames.map((_,i)=>plantColors[i%plantColors.length]), borderRadius:4 }]
  },
  options: {
    responsive:true, maintainAspectRatio:false,
    plugins:{ legend:{display:false}, tooltip:{ callbacks:{ label: ctx=>' '+fmtIDR(ctx.raw) } } },
    scales:{ x:{ ticks:{font:{size:10}} }, y:{ ticks:{ font:{size:9}, callback: v=>v>=1e9?'Rp '+(v/1e9).toFixed(1)+'B':'Rp '+(v/1e6).toFixed(0)+'M' } } },
    onClick: (evt, els) => { if(els.length) showPlantModal(pNames[els[0].index], plantColors[els[0].index%plantColors.length]); }
  }
});

function showPlantModal(plant, color){
  const pData = byPlant[plant];
  setModalHeader('Plant '+plant, 'AR Outstanding — Customer Detail', color||'#1B3A6B');
  openModal();
  const rows = allRows.filter(r => r.plant === plant);
  const totalPlantAR = rows.reduce((s,r)=>s+r.total,0);
  setModalSummary(
    summaryCard('Total AR', fmtIDR(pData.total), null, color||'#1B3A6B') +
    summaryCard('Customers', rows.length, null, 'var(--navy)') +
    summaryCard('% of Total AR', totalAR>0?(pData.total/totalAR*100).toFixed(1)+'%':'—', null, 'var(--muted)')
  );
  setModalHead(`<tr><th>#</th><th>Customer</th><th>Collector</th>
    <th class="num">Current</th><th class="num">&gt;60d Overdue</th>
    <th class="num">Total AR</th><th class="num">% of Plant</th></tr>`);
  if(!rows.length){
    setModalBody(`<tr><td colspan="7" style="text-align:center;padding:32px;color:var(--muted)">No customers found.</td></tr>`);
  } else {
    const sorted = [...rows].sort((a,b)=>b.total-a.total);
    setModalBody(sorted.map((r,i)=>{
      ...
      return `<tr style="${overdue>0?'background:#fff9f9':''}">
        ...
        <td class="num">
          <div style="display:flex;align-items:center;gap:6px;justify-content:flex-end">
            <div style="flex:1;background:#e2e8f0;border-radius:99px;height:5px;min-width:40px">
              <div style="width:${Math.min(share,100)}%;background:${color||'#1B3A6B'};height:100%;border-radius:99px"></div>
            </div>
            <span style="font-size:10px;color:var(--muted);min-width:30px;text-align:right">${share}%</span>
          </div>
        </td></tr>`;
